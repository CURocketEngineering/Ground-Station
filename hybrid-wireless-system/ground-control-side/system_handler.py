...
def user_input():
    ...
    while True:
        ...
        available_commands = [
            cmd
            for cmd, allowed_state in command_states.items()
            if allowed_state == system_state or allowed_state == "ANY"
        ]
        print(
            f"[ℹ] Available commands in {system_state} state: {', '.join(available_commands)}"
        )
        ...
        if cmd in available_commands:
            ...
            print(
                f"\t[⚠] Command '{cmd}' not available in current state '{system_state}'."
            )
        else:
...
def process_command(command):
    ...
    if command == "CMD:ARM":
        ...
        if relay_state["PURGE"] == 0:
            ...
            close_purge_relay()
        print(
            f"\t[ℹ] Purge relay toggled to {'ON' if relay_state['PURGE'] else 'OFF'}."
        )
...
def dump_nitrous():
    ...
    if response == "yes":
        print(
            "\t[ℹ] Nitrous tank valve is closed. Dumping idle nitrous between bottle and fill valve."
        )
