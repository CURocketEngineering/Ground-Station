...
def plot_stacked_summary_figure(
    ...
    first_below_2_after_first_above_2 = launch_df[launch_df["total_acceleration"] <= threshold].index[
        launch_df.index.get_loc(first_above_2) + 1:].min()

    burn_time = (first_below_2_after_first_above_2 - first_above_2) if first_above_2 and first_below_2_after_first_above_2 else 0.0
    ...
    fig = make_subplots(
        ...
        vertical_spacing=0.025, row_heights=[0.6, 0.4],
    ...
    fig.update_xaxes(
        title_text="Time from Detected Launch (s)",
        row=2, col=1
    )
