...
class LaunchDetector:
    def __init__(self, threshold_g: float = 2.0, window_size: int = 5, pre_launch_seconds: int = 10, use_all_post_launch: bool = True):
        self.threshold_g = threshold_g  # Acceleration threshold for launch detection (in G)
        self.window_size = window_size  # Moving average window size
        self.pre_launch_seconds = pre_launch_seconds
        self.use_all_post_launch = use_all_post_launch  # If True, uses all data after launch
        
    def detect_launch_from_csv(self, csv_file_path: str) -> Tuple[pd.DataFrame, int, int]:
        """
    ...
    def _find_launch_index(self, df: pd.DataFrame) -> int:
        ...
        accel_columns = ['ACCELEROMETER_Z', 'ACCELEROMETER_X', 'ACCELEROMETER_Y', 'ACCLZ', 'ACCLX', 'ACCLY']
        ...
        g_force_smooth = g_force.rolling(window=self.window_size, center=True).mean().fillna(g_force)
        ...
        if timestamp_data is not None:
            ...
            wrap_points = time_diff[time_diff < -1000].index.tolist()  # More than 1 second backwards
    ...
    def _get_launch_timestamp(self, df: pd.DataFrame, launch_index: int) -> float:
        ...
        for col in timestamp_columns:
            if col in df.columns and not df[col].isna().all():
                try:
                    timestamp = pd.to_numeric(df[col].iloc[launch_index], errors='coerce')
    ...
    def _trim_around_launch(self, df: pd.DataFrame, launch_index: int) -> pd.DataFrame:
        ...
        if timestamp_data is not None and timestamp_col:
            ...
            total_rows = len(df)
            pre_launch_rows = min(launch_index, self.pre_launch_seconds * 10)  # Estimate 10 Hz data
            ...
            if self.use_all_post_launch:
                ...
                print(f"Using all data from index {start_index} to end of file (row {total_rows})")
            else:
                end_index = total_rows
    ...
    def create_trimmed_csv(self, original_csv_path: str, output_csv_path: str) -> bool:
        ...
        try:
            trimmed_df, launch_index, launch_timestamp = self.detect_launch_from_csv(original_csv_path)
