...
def plot_selected_ids(csv_path, output_dir):
    ...
    df_accel_pivot = df_accel.pivot(
        index=CSV_TS_COLUMN, columns=CSV_ID_COLUMN, values=CSV_DATA_COLUMN
    )
    # Rename columns for clarity (optional but good practice)
    df_accel_pivot.rename(
        columns={X_ACCEL_ID: "ax", Y_ACCEL_ID: "ay", Z_ACCEL_ID: "az"}, inplace=True
    )
    ...
    )  # just makes it so a secondary axis actually exists and is a thing

    # actually graph the data read in from the csv
    fig.add_trace(
        go.Scatter(
            x=df_alt["time_sec"],
            y=df_alt[CSV_DATA_COLUMN],
            mode="lines",
            name="Altitude (m)",
    ...
    fig.add_trace(
        go.Scatter(
            x=df_fin_angle["time_sec"],
            y=df_fin_angle[CSV_DATA_COLUMN],
            mode="lines",
            name="Fin Angle (deg)",
    ...
    fig.add_trace(
        go.Scatter(
            x=df_accel_pivot["time_sec"],
            y=df_accel_pivot["total_accel"],
            mode="lines",
            name="Total Accel (m/s²)",
            line=dict(color="lightgreen", width=2.0, dash="dot"),
        ),
        secondary_y=True,
    )
    ...
    for index, row in tqdm(df_state_change.iterrows()):
        ...
        fig.add_annotation(
            ...
            showarrow=False,
            yshift=10,
            font=dict(size=9, color="magenta"),
    ...
    fig.update_layout(
        title=dict(
            text="<b>AARV Flight Performance Summary: Launch to Apogee</b>",
            font=dict(size=16, color="white"),  # title colors & writing
            x=0.5,
            xanchor="center",
        ),  # title pos
        plot_bgcolor="black",
        paper_bgcolor="black",
        font=dict(
            family="Arial, sans-serif", size=11, color="lightgrey"
        ),  # general font, gets overwritten in most cases
        ...
        legend=dict(
            x=0.01,
            y=0.99,  # what does the legend look like?
...
def main():
    ...
    df.to_csv(
        csv_save_path, index=False, header=False
    )  # Save without header if reading with header=None
